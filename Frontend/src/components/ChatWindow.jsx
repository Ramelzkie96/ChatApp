...
const ChatWindow = ({ selectedChat, messages = [], onNewMessage }) => {
    ...
    const fetchMessages = async () => {
      try {
        const res = await axios.get(
          `https://localhost:7085/api/messages/conversation?user1Id=${currentUser.id}&user2Id=${selectedChat.id}`
        );
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await axios.post("https://localhost:7085/api/messages/send", newMsg);
      setChatMessages((prev) => [...prev, res.data]);
      setMessage("");

      if (onNewMessage) onNewMessage(res.data);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      {/* Messages */}
      <div className="flex-1 px-6 py-4 bg-gray-100 flex flex-col justify-start overflow-y-auto">
